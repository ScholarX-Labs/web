...
export function EnrollModal({
  ...
  useEffect(() => {
    if (isEnrolling) {
      // Delay showing the priority window to allow main modal to close first
      const timer = setTimeout(
        () => {
          setShowPriorityWindow(true);
        },
        shouldReduceMotion ? 100 : 350,
      );
      return () => clearTimeout(timer);
    } else {
      setShowPriorityWindow(false);
    }
  }, [isEnrolling, shouldReduceMotion]);
  ...
  return (
    <>
      ...
      `}</style>
      {/* Main Enrollment Modal with Exit Animation */}
      <AnimatePresence>
        {isModalOpen && !showPriorityWindow && (
          ...
          >
            <DialogContent
              ...
              style={{
                animation: isEnrolling
                  ? "scaleOut 0.35s cubic-bezier(0.22, 1, 0.36, 1) forwards"
                  : undefined,
              }}
            >
