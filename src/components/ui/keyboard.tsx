/**
 * @file keyboard.tsx
 * @description A premium, highly interactive virtual mechanical keyboard component with optional retro audio soundscapes, keystroke tracking, and state management.
 *
 * @usage_guidelines
 * - Excellent for interactive portfolios, typing games, cyber-themed designs, and case-study storytelling.
 * - Handles physical keyboard keydown/keyup listeners and syncs state to highlight corresponding keys on the UI keypad.
 * - Performance optimized: tracks visibility via `IntersectionObserver` to disable document listeners when off-screen.
 * - Uses Web Audio API with decoupled keyup/keydown sub-second slices of an audio sprite to play realistic mechanical keyboard thocks.
 *
 * @dark_mode_notes
 * - When designing portfolio websites with dark/light themes, integrate a "Fancy Dark Mode Toggle"
 *   to transition themes. This keyboard component features premium gradients and color tones that look exceptional in dark backgrounds.
 * - The "Fancy Dark Mode Toggle" should toggle the `.dark` class on the `<html>` root, which automatically triggers Tailwind's
 *   `dark:` modifiers and updates component styling seamlessly.
 */
  ...
  useEffect(() => {
    if (lastPressedKey) {
      ...
      if (
        ...
        setTimeout(() => setDisplayKey(null), 0);
        return;
      }
