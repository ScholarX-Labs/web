/**
 * @file text-3d-flip.tsx
 * @description A premium 3D text flipping animation component that flips letters on hover.
 * Developed in accordance with Google Principal SWE guidelines.
 *
 * Design Guidelines & Theme Considerations:
 * - Employs a staggered animation delay based on customizable direction rules (first, last, center, random).
 * - Rotates characters along 3D axes (top, right, bottom, left) dynamically using circular transform geometries.
 * - Compatible with a "Fancy Dark Mode Toggle": automatically inherits local text and container colors.
 *
 * Performance & React 19 Considerations:
 * - Keeps event animations and state changes encapsulated in memoized callbacks and lifecycle hooks.
 * - Safe ref handling compliant with React 19.
 *
 * Usage:
 * ```tsx
...
export const Text3DFlip = ({
  ...
  const Tag = ElementTag as unknown as React.ComponentType<
