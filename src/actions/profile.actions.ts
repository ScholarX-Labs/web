...
const socialLinksSchema = z.object({
  githubUrl: z.string().url().refine(val => !val || /^https?:\/\//i.test(val), "URL must start with http:// or https://").optional().or(z.literal("")),
  facebookUrl: z.string().url().refine(val => !val || /^https?:\/\//i.test(val), "URL must start with http:// or https://").optional().or(z.literal("")),
  instagramUrl: z.string().url().refine(val => !val || /^https?:\/\//i.test(val), "URL must start with http:// or https://").optional().or(z.literal("")),
  twitterUrl: z.string().url().refine(val => !val || /^https?:\/\//i.test(val), "URL must start with http:// or https://").optional().or(z.literal("")),
  linkedinUrl: z.string().url().refine(val => !val || /^https?:\/\//i.test(val), "URL must start with http:// or https://").optional().or(z.literal("")),
});
