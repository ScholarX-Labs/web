...
export async function GET(request: NextRequest) {
  try {
    ...
    // Securely compare the internal key to prevent timing attacks
    const providedKey = request.headers.get("x-internal-key") || "";
    const expectedKey = process.env.INTERNAL_API_KEY || "";

    // Prevent empty key comparison from being true if expectedKey is also empty
    const providedBuf = Buffer.from(providedKey);
    const expectedBuf = Buffer.from(expectedKey);

    let isInternal = false;
    if (expectedKey.length > 0 && providedBuf.length === expectedBuf.length) {
      isInternal = crypto.timingSafeEqual(providedBuf, expectedBuf);
    }
