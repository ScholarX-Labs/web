...
function makeOpportunitiesDomain(): ExecutiveDomain {
...
test("GET opportunities and AI returns quality queue", async () => {
  const handlers = createExecutiveRouteHandlers({
    ...
    createDomain: makeOpportunitiesDomain,
  });
  ...
  assert.equal(body.data.sections.opportunityCleanupQueue.rows.length, 2);
  assert.equal(body.data.sections.opportunityQualitySummary.brokenLinks, 1);
  assert.equal(body.data.sections.opportunityQualitySummary.highSaveLowApply, 1);
});
