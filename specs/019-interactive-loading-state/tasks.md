...
- [x] T001 Create component directory structure in `src/components/ui/interactive-loader/`
- [x] T002 Verify UI dependencies (Framer Motion, Lucide React) in `package.json`
...
- [x] T003 [P] Create `LoadingSessionState`, `LoadingStage`, and `UserLoaderPreferences` interfaces in `src/components/ui/interactive-loader/types.ts`
- [x] T004 [P] Create `trivia-data.ts` in `src/components/ui/interactive-loader/trivia-data.ts` with contextual trivia banks
- [x] T005 Create `AudioHapticController.ts` in `src/components/ui/interactive-loader/AudioHapticController.ts` for Web Audio API synthesis
- [x] T006 Implement `useInteractiveLoader` hook in `src/components/ui/interactive-loader/useInteractiveLoader.ts` (depends on types, AudioHapticController)
- [x] T007 Create module exports in `src/components/ui/interactive-loader/index.ts`
...
- [x] T008 [P] [US1] Create `TriviaOverlay` component in `src/components/ui/interactive-loader/TriviaOverlay.tsx`
- [x] T009 [US1] Create main `InteractiveLoader` container in `src/components/ui/interactive-loader/InteractiveLoader.tsx` integrating `useInteractiveLoader` and `TriviaOverlay`
...
- [x] T010 [P] [US2] Create `ProgressStageTrack` component in `src/components/ui/interactive-loader/ProgressStageTrack.tsx`
- [x] T011 [US2] Integrate `ProgressStageTrack` into `src/components/ui/interactive-loader/InteractiveLoader.tsx`
...
- [x] T012 [P] [US3] Create `BubblePopGame` component in `src/components/ui/interactive-loader/BubblePopGame.tsx` utilizing Framer Motion
- [x] T013 [US3] Integrate `BubblePopGame` into `src/components/ui/interactive-loader/InteractiveLoader.tsx`
...
- [x] T014 [US4] Implement transition & success banner states in `src/components/ui/interactive-loader/InteractiveLoader.tsx`
- [x] T015 [US4] Implement error/retry fallback states in `src/components/ui/interactive-loader/InteractiveLoader.tsx`
...
- [x] T016 [P] Verify `prefers-reduced-motion` compliance in all Framer Motion components
- [x] T017 [P] Implement `aria-live="polite"` and keyboard navigation (Tab/Enter) in `TriviaOverlay` and `InteractiveLoader`
- [x] T018 Code cleanup and adherence to ScholarX styling and glassmorphism standards
