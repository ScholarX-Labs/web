...
- Q: When exactly is the certificate generated — after a single lesson or after the full course? → A: After the FULL COURSE. All individual lessons must reach the watch threshold before the course is considered complete and the certificate is issued.
- Q: What is the minimum watch percentage to consider a lesson complete? → A: 80%. A lesson is marked complete when the furthest timestamp reached equals or exceeds 80% of the total video duration.
...
- **FR-001**: The system MUST automatically issue a certificate to a participant when they satisfy the completion criteria for their enrolled program: (a) ALL lessons in the course have been watched to at least the configured minimum watch percentage, AND (b) if the course contains quizzes, they have passed all required quizzes with a score at or above the configured minimum quiz score. If a course has no quizzes, criterion (a) alone is sufficient. The system MUST NOT issue a certificate to a participant who does not meet all criteria.
- **FR-001a**: Each program (course) MUST store a `CompletionCriteria` record defining: the minimum watch percentage (0–100, default **80**), whether quizzes are required for that course (boolean), and the minimum quiz score (0–100, required when quizzes are enabled). These values MUST be configurable by an admin per course.
- **FR-001b**: A lesson is considered complete when the furthest timestamp reached by the user equals or exceeds **80%** of the total video duration. The system MUST rely on background telemetry from the video player to track this automatically, without requiring explicit user acknowledgement. When ALL lessons in a course are individually complete (and any quiz criteria are met), the **course** is marked complete and the certificate is issued instantly.
- **FR-001c**: The video telemetry MUST calculate watch percentage based on the furthest timestamp reached, regardless of whether the user scrubbed/skipped ahead. Linear viewing of every segment is not enforced.
