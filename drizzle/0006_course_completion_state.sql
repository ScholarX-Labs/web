...
CREATE TABLE IF NOT EXISTS "courses"."course_progress" (
  ...
  CONSTRAINT "course_progress_completed_lessons_chk"
    CHECK ("completed_lessons" >= 0),
  CONSTRAINT "course_progress_required_lessons_chk"
    CHECK ("required_lessons" >= 0),
  CONSTRAINT "course_progress_last_position_chk"
    CHECK ("last_position" >= 0),
  CONSTRAINT "course_progress_version_chk"
    CHECK ("version" >= 0)
);
--> statement-breakpoint
...
-- Backfill dry-run pre-check:
-- SELECT count(*) AS existing_course_progress_rows FROM "courses"."course_progress";
-- SELECT count(*) AS eligible_progress_rows FROM "courses"."lesson_progress";
--
